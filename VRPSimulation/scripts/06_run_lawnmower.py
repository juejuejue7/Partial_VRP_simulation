...
输出 : VRPSimulation/data/lawnmower.npz            逐时刻时间线
       VRPSimulation/data/lawnmower_summary.json   摘要 + 测线几何
       VRPSimulation/data/lawnmower_timeline.csv   人读版宽表
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--out", default=os.path.join(DEFAULT_DATA_DIR, "lawnmower.npz"))
...
def main() -> None:
    ...
    print("[3/3] 落盘")
    save_lawn_result(res, args.out)
    base_p = os.path.splitext(args.out)[0]
    for p in (args.out, base_p + "_summary.json", base_p + "_timeline.csv"):
