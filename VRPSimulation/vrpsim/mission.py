...
def feasibility_estimate(world: MothraWorld, cfg: MissionConfig) -> Dict[str, Any]:
    ...
    **D16 之后**(Leader 会停船等队友):这条不再是死刑判决。Leader 用自己的 DR 推算
    ...
    routes, solver = solve_minmax_vrp_from(starts, list(ds.targets_ned),
                                           time_limit_s=max(cfg.vrp_time_limit_s, 2.0),
    ...
    t_leader = world.world.x_max_m / cfg.leader_speed_mps
    ...
    return {
        ...
        "max_leader_speed_mps": float(world.world.x_max_m / t_route_lb),
...
class MissionResult:
    ...
    def leader_distance_m(self) -> float:
        """Leader 沿测线走过的距离。停船不产生位移,故与等待时长无关。"""
    ...
    def t_leader_finish_s(self) -> float:
        """Leader 走到测线终点的时刻(全域被声呐扫遍的时刻)。

        ⚠ **必须从记录取,不许用 `测线长 / v_leader` 反推**:D16 起 Leader 会停船
          等队友,两者不再相等(实际完成时刻 = 测线长/v + 累计停船时长)。
        """
        ln = self.leader_north_m
...
def run_mission(cfg: Optional[MissionConfig] = None,
    ...
    leader = Leader(cfg, track_end_north_m=mw.world.x_max_m)
    ...
    while t <= cfg.max_mission_time_s + 1e-9:
        ...
        back_north = max(0.0, leader.north - win_cfg.look_back_m)
        ...
        rec_win.append([back_north, leader.north])
...
def _hold_reason(cfg: MissionConfig, tracker: LeaderTracker, ds, id_to_idx,
                 n_targets: int, region, back_north: float,
                 held_lagging: bool) -> int:
    ...
    判据 A(`leader_wait_on_lagging_follower`):某台的推算位置落到窗口后沿之后。
        带迟滞:已经在停了的话,要重新进到后沿以北 `leader_wait_release_margin_m`
    ...
    if cfg.leader_wait_on_lagging_follower:
        thr = back_north + (cfg.leader_wait_release_margin_m if held_lagging else 0.0)
        if any(float(s.position[0]) < thr - 1e-9 for s in tracker.shadows):
            reason |= HOLD_LAGGING

    if cfg.leader_wait_on_endangered_target:
        # 再走一个前瞻期后沿会到哪:落在它以南的点就要永久出局。
        edge = back_north + cfg.leader_speed_mps * cfg.wait_lookahead_s
        occ = tracker.occupancy(n_targets)
        seen = tracker.visited_mask(n_targets, id_to_idx)
        tg = ds.targets_ned
        for j in range(n_targets):
            if seen[j] or occ[j] != WP_NONE:
                continue                        # 已观测 / 已经有人在去了
            if not window_contains_world(region, tg[j]):
                continue                        # 还没进窗口(或已经出局,停也没用)
            if float(tg[j][0]) < edge - 1e-9:
...
def _write_timeline_csv(res: MissionResult, path: str) -> None:
    ...
    cols = ["t_s", "leader_north_m", "win_back_m", "win_front_m", "visited_count"]
    for i in range(n_f):
