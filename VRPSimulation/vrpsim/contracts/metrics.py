...
是个**取最大值**,它会随 Leader 速度**换主导项**:
    ...
    L = 0.3 m/s   测线 1666.7 s  >  VRP 1084.3 s   ⇒ 分子 = 1666.7(测线主导)
    L = 0.5 m/s   测线 1000.0 s  <  VRP 1084.3 s   ⇒ 分子 = 1084.3(VRP 主导)
    L = 1.0 m/s   测线  500.0 s  <  VRP 1084.3 s   ⇒ 分子 = 1084.3(VRP 主导)
...
METRICS: Tuple[MetricSpec, ...] = (
    ...
    MetricSpec(
        ...
        definition="离理论最优还差多少。分子是 feasibility_estimate 给的乐观下界 "
                   ...
                   "L≥0.5 时由「VRP 路线 1084.3 s」主导 —— 换了主导项就不再是"
    ...
    MetricSpec(
        ...
        definition="**本族的要害指标。** 它测的是「有机先干完、然后干等着」—— "
                   ...
                   "稼働時間自然都约等于任务时长(提案就是这种情况,"
                   "距離 2.8% / 時間 0.7%,原因是逐次重规划持续把活摊开)。",
        better=LOWER_BETTER, family=FAMILY_BALANCE, fmt="{:.1%}"),
